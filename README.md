...
| `serve` | Host Oil Desk UI + `/api/cars` on `127.0.0.1:4739` from the **embedded** static export (`web/out`). **No npm/Node required.** `[--addr]` `[--mirror]` `[--web-dir]`. **History** (`/history/`): one fill is one block; drag onto a car; SQLite `assignment_events` logs PDI-0003 → PDI-0020. Rebuilds do not wipe owner calls. **Devices** is GPS: cached evidence + one-box / one-fill live probe. Button **Apply saved OneStep device information** reads `data/runtime/device-information.json` (no live `/device`). |
