...
Who owns what: [`docs/DATA-OWNERS.md`](docs/DATA-OWNERS.md). OneStep places (Gas Stations only, portal-first writes): [`docs/ONESTEP-PLACES-API.md`](docs/ONESTEP-PLACES-API.md). Mileage box score: recorded = gas card punch; expected = good maintenance + drive-stop (never OneStep odometer).
