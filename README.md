...
| Cmd | What |
|---|---|
| `sync-enterprise` | File drop (`--vehicles` `--fuel-details` …) first; else Chrome session via `EFLEETS_CDP_URL` + Network-captured `EFLEETS_*_URL` (no password typing); password HTTP last. Oil/lube shop ROs seed last oil. Does not compute Last Reading. |
