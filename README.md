...
Without Supabase credentials the CLI writes a **mock mirror** at `web/data/cars.json` and `oilchange serve` (or Next `/api/cars`) serves that. With credentials, sync upserts into `fleet_cars`. Schema/RLS: `supabase/migrations/` (anon SELECT on `fleet_cars` only; other `fleet_*` tables exist for a later full copy and deny anon). Canonical sqlite/pg schema is `migrations/001_schema.sql`–`011_place_catalog.sql`.
