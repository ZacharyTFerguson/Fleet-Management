...
| `serve` / `desk` | Oil Desk UI + `/api/cars` from the embedded export. `[--addr]` `[--mirror]`. `oilchange desk` is `serve --app`: Chrome/Edge window, no address bar. Phone: Add to Home Screen (standalone PWA). **History** (`/history/`): drag one fill onto one car; audit in `assignment_events`. **Devices** is the GPS surface (cached evidence + one-box probe). **Login / Status / Secrets / Stations / Box score**: desk session, connection health, AES-GCM vault (never echo secrets), Gas_Stations-only download (`GET /zone-group` then `/zone`), portal-first create until `ONESTEP_WRITE_PROVEN=1`, mileage box score (gas card vs maintenance + drive-stop). **Impreza oil spec** (Oil Desk sub-feature — disclosure on `/`, deep link `/impreza/`): 2024 Impreza oil-change-by-use + engine diagram; Warranty & Maintenance Booklet Note 1 — normal 6,000 mi / 6 mo, fleet 5,000 mi, severe 3,000 mi / 3 mo; manual spec, never Last Reading. Button **Apply saved OneStep device information** (`POST /api/devices/vin-from-file`) pairs from `data/runtime/device-information.json` with no live `/device`. Owner assignments survive `cards rebuild`. Never Last Reading. Secrets stay in `oilchange.env` or the vault. |
