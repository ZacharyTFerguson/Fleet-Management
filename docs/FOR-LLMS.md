...
| `boxscore` | Rebuild/list the mileage ledger. Recorded = gas card transaction odo + punch time. Expected = good maintenance odo + drive-stop since that stamp. Suspect/HOLD visible, excluded from trend. Never invent miles. Never Last Reading. |
...
5a. **Mileage box score.** Expected @ gas card transaction = last good maintenance odo + OneStep drive-stop miles since that maintenance timestamp. Recorded = gas card transaction odo + punch time. Never use OneStep odometer as the base. Ledger stores overage / shortage / abs_diff. Trend uses trusted rows only; weird punches are Suspect/HOLD (visible, out of trend until corrected or dismissed).
