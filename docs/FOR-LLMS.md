...
| `cards watch` | Targeted loop of those leftovers: per card, newest 10 swipes (then the rest of the watch list). Drive-stop only the **watched** `factory_id`s (1-mile hits + Virginia recorded vehicle + one hypothesis roster car). `--pace 35s` between GETs; `Retry-After` wins. Coverage for persist is the watched set, not the whole fleet. Same persist locks as nearby. Never a 260-box pull. |
