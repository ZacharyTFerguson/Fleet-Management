...
| `boxscore` | Rebuild/list the mileage ledger. Recorded mileage = **gas card transaction** (fuel punch odo + Provider Transaction Time). Expected = good maintenance odo + drive-stop since that stamp. Suspect/HOLD bucket visible, excluded from trend until corrected/dismissed. Never invent miles. Never Last Reading. |
...
5a. **Mileage box score.** Recorded mileage = the **gas card transaction** (not a vague Enterprise reading). Expected = good maintenance odo + OneStep drive-stop since that stamp. Trusted gas card transactions feed overage/shortage and whether `|expected−recorded|` goes up or down. Weird punches sit in a Suspect/HOLD bucket and do not move the trend until corrected or dismissed.
