...
**oilchange** is a Go CLI (`./cmd/oilchange`) plus an embedded **Oil Desk** UI for one fleet’s oil-change, fuel-card, and GPS desk.
...
1. **Last Reading** — trusted miles for oil interval (when is the next change due).
...
Secrets live in gitignored **`oilchange.env`** (template: `oilchange.env.example`) or **Cloud Agent secrets**. Never paste credentials, PEMs, JWTs, portal passwords, or connection strings into chat, PRs, or git.
...
- **Never ask a human to log into eFleets in chat.** File-drop `--vehicles` / `--fuel-details` does not need a portal login. Already-open Chrome session (CDP) is documented separately; do not type a password for that path.
- Tests never hit live eFleets, OneStep, or Supabase.

Do not commit `oilchange.env`, sqlite files, `data/runtime/`, PEMs, or `cards.json`.
