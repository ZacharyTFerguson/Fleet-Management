...
| `cards watch` | Targeted loop of those leftovers: per card, newest 10 swipes (then the rest of the watch list). Drive-stop only the **watched** `factory_id`s (1-mile hits + newest Virginia recorded vehicle + one hypothesis roster car). `--pace 35s` between GETs; `Retry-After` wins. `--virginia` limits the loop to VA recorded vehicles (rental labels are not VA). `--skip-vin` skips the post-watch AskEmpty `/device` VIN GETs. Without `--skip-vin`, GET `/device?device_id=&latest_point=true` for OBD VIN on leftover unpaired boxes and join exact 17-char `device_state.vin` = `cars.vin`. Coverage for persist is the newest VA seed box, not 1-mile spectators. Same persist locks as nearby. Never a 260-box pull. After a VA/watched pull, rematch with `cards rebuild --no-gps` so GPS-first does not unpaced-fetch the rest of the linked fleet. |
...
| `serve` / `desk` | Oil Desk UI + `/api/cars` from the embedded export. `[--addr]` `[--mirror]`. `oilchange desk` is `serve --app`: Chrome/Edge window, no browser chrome. Phone: Add to Home Screen (standalone PWA). **History** (`/history/`): drag one fill onto one car; audit in `assignment_events`. **Devices** is the GPS surface (cached evidence + one-box probe). **Login / Status / Secrets / Stations / Box score**: desk session, connection health, AES-GCM vault (never echo secrets), Gas_Stations-only download (`GET /zone-group` then `/zone`), portal-first create until `ONESTEP_WRITE_PROVEN=1`, mileage box score (gas card vs maintenance + drive-stop). Button **Apply saved OneStep device information** (`POST /api/devices/vin-from-file`) pairs from `data/runtime/device-information.json` with no live `/device`. Owner assignments survive `cards rebuild`. Never Last Reading. Secrets stay in `oilchange.env` or the vault. |
| `boxscore` | Rebuild/list the mileage ledger. Recorded = **gas card transaction** at Provider Transaction Time. Expected = last good maintenance odo + drive-stop since that stamp (never last oil + interval). Difference = recorded − expected; also `abs_diff`. HOLD when maint or OneStep pairing is missing. Never invent miles. Never Last Reading. |
...
5a. **Mileage box score.** When a gas card / fuel punch is recorded: **expected** = last good maintenance odo (trusted shop/RO) + OneStep drive-stop since that stamp (`device_id` + `dt_from`/`dt_to`). **Recorded** = the gas card / Enterprise fuel punch at Provider Transaction Time. **Difference** = recorded − expected (overage if card ahead; shortage if card behind); also store `abs_diff`. Trend is whether `|gap|` is growing (worse vs maint+GPS) or shrinking (improving). Last Reading on the sheet stays Ent last-good @ a known second + OneStep since that second — a different number. Last oil + interval is **not** box-score expected. HOLD / skip scoring when the maintenance base or OneStep pairing is missing — never invent miles, never OneStep odometer as the base. Suspect/HOLD stay visible and out of trend until corrected or dismissed.
