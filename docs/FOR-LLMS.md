...
| `cards nearby` | Unknown-card hunt via Nearby Address rules: station, **provider fill second** (not bank posting), Eastern day-before through day-after, devices within **1 mile**. Watch until exclusive at fill time (2 likely / 3 certain linked car). `--report` queues OneStep `near_address`. `--persist` writes certain **linked** car eras only. Never Last Reading. Never invent unpaired `factory_id`. |
