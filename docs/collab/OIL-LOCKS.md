...
7. SQLite is the working store. Neon is one vendor backup of sqlite tables. This fleet’s Supabase is Oil Desk today (`fleet_cars`); it is the planned **second vendor full backup**. Do **not** add a third store, extra Neon, or S3. Refuse XRAY (`chjqcznyxvtjbamttqdj`) and `-pooler` as `DATABASE_URL`. Do not treat today’s `fleet_cars` sync as that full backup until a later command copies the same sqlite tables Supabase-side.
