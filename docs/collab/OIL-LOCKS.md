...
5. Join cars to GPS boxes on **`factory_id`**. `device_id` is for History / drive-stop. **`display_name` is never a join key.** Unpaired boxes may attach by exact 17-char OBD VIN (`device_state.vin` = `cars.vin`), not plate or nickname.
