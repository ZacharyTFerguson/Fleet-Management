...
- SQLite (`OILCHANGE_DB`) is the daily driver. Neon is one vendor backup. Supabase `fleet_cars` is the Oil Desk remote today; that same project is the planned second-vendor full backup (do not add extra Neon, S3, or a third store). `--no-remote` when you must not push null Last Reading.
