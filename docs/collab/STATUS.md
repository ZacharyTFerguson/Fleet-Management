...
Updated: 2026-09-11 ~01:20 UTC (Grok VA card↔GPS audit on `cursor/va-card-gps-audit-1f1f`). **Do not** re-run the 260-box nearby `--live`. Do not copy this Cloud Agent sqlite over the operator desktop db.

This pod had **no** inherited `gps-stops.json`. After VA-only watch (`--virginia --skip-vin --fills 10 --pace 35s`) + `cards rebuild --no-gps`: device-link **195 / 205 (95.1%)**, known **22 / 205 (10.7%)**, VA boxes **35 / 35**, Last Reading still **NULL**. Operator desktop (2026-09-04) remains **92 / 205 (44.9%)** with the 146k-visit cache. Full write-up: [`VA-CARD-GPS-AUDIT.md`](VA-CARD-GPS-AUDIT.md). Composer owns pair-date schema on `cursor/card-pair-dates-1f1f`.

Prior 2026-09-05: added [`OIL-CHANGES-NOT-REFLECTED.md`](OIL-CHANGES-NOT-REFLECTED.md). Prior 2026-09-04: watch `--live --persist` finished, then OneStep VIN-ask on leftover unpaired boxes, then `cards history --no-gps` rematch. Known on **operator desktop** sqlite after rematch: **92 / 205 (44.9%)**. VIN-ask linked **0** new boxes. Last Reading untouched on desktop. When OneStep is cooling down, use saved Device Information JSON (`devices vin --from`, Oil Desk **Apply saved OneStep device information**) instead of another live `/device`.
...
- `oilchange cards watch [--live] [--persist] [--virginia] [--skip-vin] [--fills 10] [--pace 35s]` loops unknown cards: newest 10 punches, drive-stop **only watched** `factory_id`s (VA seed + 1-mile hits). `--virginia` limits to Virginia recorded vehicles; `--skip-vin` skips the post-watch AskEmpty `/device` VIN. Without `--skip-vin`, the hunt asks OneStep `GET /device?device_id=&latest_point=true` for OBD VIN on unpaired watched boxes and joins exact 17-char `device_state.vin` = `cars.vin`. `oilchange devices vin` is the same VIN ask for the whole unpaired set. When OneStep is cooling down, save Device Information JSON to `data/runtime/device-information.json` and click **Apply saved OneStep device information** on Oil Desk (or `devices vin --from`) — no live `/device`. Then `cards history --no-gps` rematches GPS-at-the-pump. Never Last Reading. Never `display_name`.
