...
Updated: 2026-09-11 (UTC) — OneStep jwt-rs256 GPS pull on this pod: `devices sync` + `cards watch --virginia --live --skip-vin --pace 35s` then paced `cards watch --live --skip-vin --persist --pace 35s` (watched boxes only). Rematch `cards rebuild --no-gps`. Known **36 / 205 (17.6%)** vs this pod’s prior cache-only **22 / 205**. Last Reading still null. **Do not** re-run the 260-box nearby `--live`. Rental labels are not VA19: [`RENTAL-VA19-ALIAS.md`](RENTAL-VA19-ALIAS.md).
...
- `oilchange cards watch [--live] [--persist] [--virginia] [--skip-vin] [--fills 10] [--pace 35s]` loops unknown cards: newest 10 punches, drive-stop **only watched** `factory_id`s (VA seed + 1-mile hits). `--virginia` limits the loop to VA recorded vehicles (rental labels are not VA). `--skip-vin` skips AskEmpty `/device` VIN GETs after GPS. Without `--skip-vin`, the hunt asks OneStep `GET /device?device_id=&latest_point=true` for OBD VIN on unpaired watched boxes and joins exact 17-char `device_state.vin` = `cars.vin`. `oilchange devices vin` is the same VIN ask for the whole unpaired set. When OneStep is cooling down, save Device Information JSON to `data/runtime/device-information.json` and click **Apply saved OneStep device information** on Oil Desk (or `devices vin --from`) — no live `/device`. After a VA/watched pull, rematch with `cards rebuild --no-gps` / `cards history --no-gps` so GPS-first does not unpaced-fetch the rest of the linked fleet. Never Last Reading. Never `display_name`.
...
- **Rental 1 / Rental 2** are rental-car `display_name` labels (live `factory_id` `351358810724200` / `350457794656486`), not VA15/VA19. VA19 control is testdata **CARD-19** (one swipe on `27VA19`); live Lindsey card `xxxxxxxxxxxxx93125` on `27SGXP`. `VA RENTAL 2` is not a Virginia watch seed. See [`RENTAL-VA19-ALIAS.md`](RENTAL-VA19-ALIAS.md).
...
**This Cloud Agent pod (2026-09-11):** OneStep auth jwt-rs256 (API key + PEM present; no values in chat). `devices sync` upserted **264** boxes (no miles). VA watch: **3** drive-stop GETs, `failed=0`. Paced leftover watch: **47** drive-stop GETs, `failed=0`, 87 unknown cards, not a fleet pull. Cache **14,862** visits / **65** `factory_id`s (`with_pos=14756`, pumps=489). `cards rebuild --no-gps`: gps-first matches=516 best=33, known **36 / 205 (17.6%)**, device_link **195 / 205 (95.1%)**, ladder_locked=31. vs this pod before the live pull: **22 / 205 (10.7%)**. vs code target 95% and owner 99.5%: short. Last Reading null on all 205; `drive_stop_miles` still 0. Did **not** GPS-first rebuild with OneStep attached (that unpaced-fetches missing linked boxes). 137 linked boxes remain out of cache; watch seeds 1-mile hits first so 6 leftover recorded-car boxes were not asked. TESTdata CARD-19 is not in this live DETAILS window.

2026-09-04 later run (real Drive `DETAILS_583424_30-Days` + live OneStep stops, May 16–Jun 17 window, then VIN-linked boxes filled into the cache): **35,404** stop windows (`with_pos=35389`), **1,421** pump clusters, **214** GPS-first matches, **63** BEST, **1,098** calls, **119** geocoded stations. Adding boxes reduced exclusive sits (251 → 214) and still raised known cars. Nearby `--live` later filled 260 uncovered boxes into the same gitignored cache (**72,074** visits / **1,737** pumps) without rewriting `card_eras`. Watch `--live` then filled **watched** boxes only (cache **77,785** → **146,771** visits / **2,547** pumps). `cards history --no-gps` rematch: **522** GPS-first matches, **92** BEST, known **92 / 205 (44.9%)**. Synthetic TRACKER 10:00 AM file-drop is no longer the coverage input. That **92 / 205** was a different sqlite/cache (full nearby+watch), not this pod’s 36/205.
